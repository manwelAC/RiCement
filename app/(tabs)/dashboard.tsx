...
interface Project {
  ...
  pouringTime?: number; // in seconds, for pouring countdown (8 seconds)
...
export default function DashboardScreen() {
  ...
  const [isManualProjectModalVisible, setIsManualProjectModalVisible] =
    useState(false);
  ...
  const temperatureRef = useRef<any>(null);

  // Load projects data and calculate RHB totals (manual_projects only)
  const loadRHBData = useCallback(async () => {
    try {
      const user = auth.currentUser;
      if (!user) return;

      // Query completed manual projects from Firebase
      const manualQ = query(
  ...
  const filterRHBDataByDate = useCallback(async (from: string, to: string) => {
    try {
      ...
      Alert.alert(
        "Error",
        "Failed to filter data. Please check your date format.",
      );
    }
  }, []);
  ...
  const handleCreateManualProject = async () => {
    if (
      !manualProjectName.trim() ||
      !manualProjectBlocks.trim() ||
      !manualProjectTime.trim()
    ) {
    ...
    try {
      ...
      const manualProjectData = {
        ...
        isManual: true,
        timerActive: false, // Will be activated after 2 second delay
      ...
      Alert.alert(
        "Success",
        "Ang manual project ay nag-sisimula na, Ang timer ay guamgana na.",
      );
    } catch (error: any) {
  ...
  // Temperature cycling loop
  useEffect(() => {
    const temperatureCycle = [
      { temperature: 32, duration: 10000 }, // 10 seconds
      { temperature: 33, duration: 5000 }, // 5 seconds
      { temperature: 31, duration: 10000 }, // 10 seconds
      { temperature: 29, duration: 10000 }, // 10 seconds
      { temperature: 34, duration: 10000 }, // 10 seconds
      { temperature: 30, duration: 10000 }, // 10 seconds
    ];
    ...
    const startCycle = () => {
      const updateTemperature = () => {
        const current = temperatureCycle[currentIndex];
        setCurrentTemperature(current.temperature);

        temperatureRef.current = setTimeout(() => {
          currentIndex = (currentIndex + 1) % temperatureCycle.length;
          updateTemperature();
        }, current.duration);
      };
    ...
    return () => {
      if (temperatureRef.current) clearTimeout(temperatureRef.current);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    // Listen to all non-completed manual projects
    const q = query(
      collection(db, "manual_projects"),
      where("userId", "==", user.uid),
      where("status", "in", ["Pouring", "Mixing", "Pouring2"]),
    );
      ...
      timerRef.current = setInterval(async () => {
        setRemainingTime((prev) => {
          ...
          if (activeManualProject.id) {
            if (
              activeManualProject.status === "Pouring" ||
              activeManualProject.status === "Pouring2"
            ) {
              updateDoc(doc(db, "manual_projects", activeManualProject.id), {
                pouringTime: newTime,
              }).catch((err) =>
                console.error("Error updating pouring timer:", err),
              );
            } else if (activeManualProject.status === "Mixing") {
              updateDoc(doc(db, "manual_projects", activeManualProject.id), {
                remainingTime: newTime,
              }).catch((err) =>
                console.error("Error updating mixing timer:", err),
              );
            } else if (activeManualProject.status === "Molding") {
              updateDoc(doc(db, "manual_projects", activeManualProject.id), {
                dispenseTime: newTime,
              }).catch((err) =>
                console.error("Error updating molding timer:", err),
              );
            }
          }
            ...
            if (activeManualProject.id) {
              if (activeManualProject.status === "Pouring") {
                ...
                updateDoc(doc(db, "manual_projects", activeManualProject.id), {
                  ...
                  up: true, // Machine UP during initial mixing
                  down: false,
                }).catch((err) =>
                  console.error("Error moving to Mixing:", err),
                );
              } else if (activeManualProject.status === "Mixing") {
                // Move to Pouring2 (20 seconds) - Pour out mixture while pump continues
                updateDoc(doc(db, "manual_projects", activeManualProject.id), {
                  status: "Pouring2",
                  remainingTime: 0,
                  pouringTime: 20,
                  timerActive: true, // Pump continues during Pouring2
                  pouringActive: true, // Pouring out the mixed product
                  up: false, // Machine DOWN - pour out the mixture
                  down: true,
                }).catch((err) =>
                  console.error("Error moving to Pouring2:", err),
                );
              } else if (activeManualProject.status === "Pouring2") {
                // Pouring2 complete, check if all blocks are done
                const completedBlocks =
                  (activeManualProject.completedBlocks || 0) + 1;

                if (completedBlocks >= activeManualProject.blocks) {
                  // All blocks completed
                  updateDoc(
                    doc(db, "manual_projects", activeManualProject.id),
                    {
                      timerActive: false,
                      pouringActive: false,
                      status: "Completed",
                    ...
                    .then(() => {
                      // Ensure timerActive is definitely off
                      updateDoc(
                        doc(db, "manual_projects", activeManualProject.id),
                        { timerActive: false },
                      ).catch((err) =>
                        console.error(
                          "Error deactivating timer on completion:",
                          err,
                        ),
                      );
                      ...
                      Alert.alert(
                        "Process Complete",
                        `Manual project "${activeManualProject.name}" has finished all ${activeManualProject.blocks} blocks!`,
                      );
                    })
                    .catch((err) =>
                  ...
                  updateDoc(
                    ...
                    .then(() => {
                      console.log(
                        `Starting block ${completedBlocks + 1} of ${activeManualProject.blocks}`,
                      );
                    })
                    .catch((err) =>
                      console.error("Error starting next block:", err),
                    );
                }
              }
            }
    ...
    >
      ...
      >
            ...
            <View style={styles.timerHeader}>
              <View style={styles.timerTitleContainer}>
                ...
                <ThemedText style={styles.timerTitle}>
                  Active: {activeManualProject.name}
                </ThemedText>
              </View>
            ...
            <View style={styles.timerDetails}>
              <View style={styles.timerDetailItem}>
                ...
                <ThemedText style={styles.timerDetailText}>
                  Block {(activeManualProject.completedBlocks || 0) + 1}/
                  {activeManualProject.blocks}
                </ThemedText>
              </View>
              <View style={styles.timerDetailItem}>
                ...
                <ThemedText style={styles.timerDetailText}>
                  {activeManualProject.status === "Pouring"
                    ? "Pouring Materials"
                    : activeManualProject.status === "Mixing"
                      ? "Mixing"
                      : activeManualProject.status === "Pouring2"
                        ? "Final Pouring"
                        : activeManualProject.status === "Molding"
                          ? "Molding Block"
                          : "Processing"}
                </ThemedText>
              </View>
            ...
            <View style={styles.progressBarContainer}>
              <View
                style={[
                  styles.progressBar,
                  {
                    width: `${rawMaterials.rha}%`,
                    backgroundColor: getColorForPercentage(rawMaterials.rha),
                  },
                ]}
              />
            </View>
            <ThemedText style={styles.materialPercent}>
              {rawMaterials.rha}%
            </ThemedText>
          </View>
          <View style={styles.materialItem}>
            ...
            <View style={styles.progressBarContainer}>
              <View
                style={[
                  styles.progressBar,
                  {
                    width: `${rawMaterials.cement}%`,
                    backgroundColor: getColorForPercentage(rawMaterials.cement),
                  },
                ]}
              />
            </View>
            ...
            <View style={styles.progressBarContainer}>
              <View
                style={[
                  styles.progressBar,
                  {
                    width: `${rawMaterials.sand}%`,
                    backgroundColor: getColorForPercentage(rawMaterials.sand),
                  },
                ]}
              />
            </View>
            ...
            <View style={styles.progressBarContainer}>
              <View
                style={[
                  styles.progressBar,
                  {
                    width: `${rawMaterials.water}%`,
                    backgroundColor: getColorForPercentage(rawMaterials.water),
                  },
                ]}
              />
            </View>
        ...
        <ThemedView
          ...
          darkColor="transparent"
        >
          <Pressable style={styles.startButton} onPress={handleStartProcess}>
        ...
        {/* Manual Controller Preview Card */}
        <Pressable
          style={styles.controllerPreviewCard}
          onPress={() => router.push("/(tabs)/controller")}
        >
          <View style={styles.controllerPreviewHeader}>
            <View style={styles.controllerPreviewTitleContainer}>
              <Ionicons name="game-controller" size={24} color="#FFFFFF" />
              <ThemedText style={styles.controllerPreviewTitle}>
                Manual Controller
              </ThemedText>
            </View>
            <Ionicons name="chevron-forward" size={24} color="#FFFFFF" />
          </View>
          <ThemedText style={styles.controllerPreviewDescription}>
            Kontrolin ang makina nang direkta
          </ThemedText>
          <View style={styles.controllerPreviewFeatures}>
            ...
            <View style={styles.featureItem}>
              ...
              <ThemedText style={styles.featureText}>Settings</ThemedText>
            </View>
          </View>
        </Pressable>
        ...
        <View style={styles.statusContainer}>
          ...
          >
            <View style={styles.temperatureBackground}>
              <View style={styles.temperatureWrapper}>
                ...
                <ThemedText style={styles.temperatureLabel}>
          ...
          <Pressable
            style={styles.statusBox}
            onPress={() => setIsRHBModalVisible(true)}
          >
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <ScrollView style={styles.manualProjectForm}>
                ...
                <View style={styles.formGroup}>
                  <ThemedText style={styles.formLabel}>
                    Estimated na Oras (Auto-calculated)
                  </ThemedText>
                  <TextInput
                ...
                <Pressable
                  style={[
                    ...
                    isCreatingManualProject &&
                      styles.createProjectButtonDisabled,
                  ]}
                ...
                >
                  {isCreatingManualProject ? (
                    ...
                    <ThemedText style={styles.createProjectButtonText}>
                      Simulan ang Process
                    </ThemedText>
                  )}
                </Pressable>
              ...
              <ScrollView style={styles.materialsDetailsContainer}>
                <View style={styles.materialsCard}>
                  <View style={styles.materialDetailItem}>
                    <ThemedText style={styles.materialDetailLabel}>
                      RHA
                    </ThemedText>
                    <View style={styles.materialDetailBar}>
                      <View
                        style={[
                          styles.materialDetailProgress,
                          {
                            width: `${rawMaterials.rha}%`,
                    ...
                    <ThemedText style={styles.materialDetailPercent}>
                      {rawMaterials.rha}%
                    </ThemedText>
                  </View>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.rhbModalContent}>
              <View style={styles.rhbModalHeader}>
                <View style={{ flex: 1 }} />
                <ThemedText style={styles.rhbModalTitle}>
                  RHA Hollow Blocks
                </ThemedText>
                <Pressable
                  onPress={() => {
              ...
              <ScrollView
                style={styles.rhbDetailsContainer}
                showsVerticalScrollIndicator={false}
              >
                {/* Quick Stats Tabs */}
                <View style={styles.rhbQuickStatsContainer}>
                  <Pressable
                    style={[
                      styles.rhbQuickStatItem,
                      !isDateFilterActive && styles.rhbQuickStatActive,
                    ]}
                    onPress={() => resetDateFilter()}
                  >
                    <ThemedText
                      style={[
                        styles.rhbQuickStatValue,
                        !isDateFilterActive && styles.rhbQuickStatValueActive,
                      ]}
                    >
                      {totalRHBMonth}
                    </ThemedText>
                    <ThemedText
                      style={[
                        styles.rhbQuickStatLabel,
                        !isDateFilterActive && styles.rhbQuickStatLabelActive,
                      ]}
                    >
                      Ngayong Buwan
                    </ThemedText>
                ...
                <View style={styles.rhbFilterSection}>
                  <ThemedText style={styles.rhbFilterSectionTitle}>
                    Custom na Petsa
                  </ThemedText>
                  ...
                  <View style={styles.rhbDateInputsRow}>
                    ...
                    <View style={styles.rhbDateInputField}>
                      <ThemedText style={styles.rhbDateLabel}>
                        Hanggang
                      </ThemedText>
                      <TextInput
                        style={styles.rhbDateInputCompact}
                  ...
                  <View style={styles.rhbFilterActionsRow}>
                    <Pressable
                      style={styles.rhbFilterButtonCompact}
                      onPress={applyDateFilter}
                    >
                      <ThemedText style={styles.rhbFilterButtonCompactText}>
                        I-Apply
                      </ThemedText>
                    </Pressable>

                    <Pressable
                      style={styles.rhbResetButtonCompact}
                      onPress={resetDateFilter}
                    >
                      <ThemedText style={styles.rhbResetButtonCompactText}>
                        Burahin
                      </ThemedText>
                    </Pressable>
                  </View>
                ...
                {isDateFilterActive ? (
                  <View style={styles.rhbResultCard}>
                    ...
                    <ThemedText style={styles.rhbResultValue}>
                      {filteredRHBData}
                    </ThemedText>
                    <ThemedText style={styles.rhbResultLabel}>
                      Nagawang RHB
                    </ThemedText>
                  </View>
                ) : (
              ...
              <ScrollView style={styles.tempDetailsContainer}>
                <View style={styles.tempInfoCard}>
                  ...
                  <View style={styles.tempValueContainer}>
                    <ThemedText style={styles.tempModalValue}>
                      {currentTemperature}°C
                    </ThemedText>
                  </View>
                  <View style={styles.tempStatusContainer}>
                    <Ionicons name="thermometer" size={24} color="#3498DB" />
                    <ThemedText style={styles.tempStatusText}>
                      Normal pa ang temperatura
                    </ThemedText>
                  </View>
                </View>
              </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // Manual Controller Preview Card Styles
  ...
  controllerPreviewHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    marginBottom: 12,
  },
  controllerPreviewTitleContainer: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
  },
  controllerPreviewTitle: {
    fontSize: 18,
    fontWeight: "700",
    color: "#FFFFFF",
    letterSpacing: 0.3,
  },
  controllerPreviewDescription: {
    fontSize: 14,
    color: "#BDC3C7",
    fontWeight: "500",
    marginBottom: 16,
  },
  controllerPreviewFeatures: {
    flexDirection: "row",
    gap: 12,
  },
  featureItem: {
