...
export default function TabLayout() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      console.log(
        "Firebase user:",
        firebaseUser ? firebaseUser.email : "No user logged in",
      );
  ...
  useEffect(() => {
    ...
    const tabsRoute = navigationState.routes.find(
      (route: any) => route.name === "(tabs)",
    );
    if (tabsRoute?.state?.routes) {
      const currentIndex = tabsRoute.state.index;
      if (typeof currentIndex === "number" && currentIndex >= 0) {
        const currentRouteName =
          tabsRoute.state.routes[currentIndex]?.name || "";
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {currentRoute !== "admin" && currentRoute !== "controller" && (
        <GlobalAIChat />
      )}
    </View>
  );
