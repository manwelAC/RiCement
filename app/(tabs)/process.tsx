...
export default function ProcessScreen() {
  ...
  const [backloadingRecords, setBackloadingRecords] = useState<
    BackloadingRecord[]
  >([]);
  const [activeTab, setActiveTab] = useState<"projects" | "inventory">(
    "projects",
  );
  const [inventorySubTab, setInventorySubTab] = useState<
    "stock" | "loading" | "backloading"
  >("stock");
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [isRawMaterialsModalVisible, setIsRawMaterialsModalVisible] =
    useState(false);
  const [isLoadingModalVisible, setIsLoadingModalVisible] = useState(false);
  const [isBackloadingModalVisible, setIsBackloadingModalVisible] =
    useState(false);
  ...
  const [selectedLoadingInventoryId, setSelectedLoadingInventoryId] =
    useState("");
  const [selectedBackloadingInventoryId, setSelectedBackloadingInventoryId] =
    useState("");
  const [showLoadingInventoryDropdown, setShowLoadingInventoryDropdown] =
    useState(false);
  const [
    showBackloadingInventoryDropdown,
    setShowBackloadingInventoryDropdown,
  ] = useState(false);
  ...
  const [companyUsers, setCompanyUsers] = useState<
    Omit<Collaborator, "blocksContributed" | "joinedAt">[]
  >([]);
  ...
  const [isProjectDetailsModalVisible, setIsProjectDetailsModalVisible] =
    useState(false);
  const [isProjectActionSheetVisible, setIsProjectActionSheetVisible] =
    useState(false);
      ...
      const unsubscribe = onValue(
        ...
        (snapshot) => {
          ...
          setMaterialLevels((prev) => ({
            ...prev,
            rha:
              typeof data.sensor_1?.distance_percent === "number"
                ? data.sensor_1.distance_percent
                : prev.rha,
            cement:
              typeof data.sensor_2?.distance_percent === "number"
                ? data.sensor_2.distance_percent
                : prev.cement,
            sand:
              typeof data.sensor_3?.distance_percent === "number"
                ? data.sensor_3.distance_percent
                : prev.sand,
            water:
              typeof data.sensor_4?.distance_percent === "number"
                ? data.sensor_4.distance_percent
                : prev.water,
          }));
        },
        (error) => {
  ...
  // Per block: 32s Pouring (Sand 8 + Cement 8 + RHA 8 + Water 8) + 60s Mixing + 10s Pouring2 + 26s Mixing2 = 128 seconds per block
  const calculateEstimatedTime = (numberOfBlocks: number): string => {
    const secondsPerBlock = 128; // 32 + 60 + 10 + 26
      ...
      setProjects((prevProjects) => {
        ...
        let updatedProjects = prevProjects.map((project) => {
          ...
          if (
            ...
            project.pouringTime !== undefined &&
            project.pouringTime > 0
          ) {
            hasChanged = true;
            const newPouringTime = project.pouringTime - 1;

            // Determine which material is currently being dispensed (8 seconds each)
            // Sand: 0-8s, Cement: 8-16s, RHA: 16-24s, Water: 24-32s = 32 total
            let currentMaterial: "sand" | "cement" | "rha" | "water" = "sand";
            if (newPouringTime > 24)
              currentMaterial = "sand"; // 32-24 = 8 remaining
            else if (newPouringTime > 16)
              currentMaterial = "cement"; // 24-16 = 8 remaining
            else if (newPouringTime > 8)
              currentMaterial = "rha"; // 16-8 = 8 remaining
            else currentMaterial = "water"; // 8-0

            // Update Firebase with new pouring time every second
            firebaseService
              .updateProject(project.id, {
                pouringTime: newPouringTime,
                currentMaterial,
              } as any)
              .catch((error) =>
                console.error(
                  "Error updating pouring time in Firebase:",
                  error,
                ),
              );

            // If pouring time reaches 0, move to Mixing (60 seconds for first mixing phase)
            if (newPouringTime <= 0) {
              firebaseService
                .updateProject(project.id, {
                  status: "Mixing",
                  pouringTime: 0,
                  remainingTime: 60, // 60 seconds for first mixing phase
                ...
                .then(() =>
                  console.log(
                    `Project ${project.id} moved to Mixing in Firebase`,
                  ),
                )
                .catch((error) =>
                  console.error(
                    "Error updating project status in Firebase:",
                    error,
                  ),
                );
          ...
          // Handle Mixing (60 seconds - first mixing phase)
          if (
            project.status === "Mixing" &&
            project.remainingTime !== undefined &&
            ...
            firebaseService
              ...
              .catch((error) =>
                console.error(
                  "Error updating remaining time in Firebase:",
                  error,
                ),
              );
            ...
            if (newRemainingTime <= 0) {
              firebaseService
                ...
                .then(() =>
                  console.log(
                    `Project ${project.id} moved to Pouring2 in Firebase`,
                  ),
                )
                .catch((error) =>
                  console.error(
          ...
          if (
            ...
            // Update Firebase with new pouring time every second
            firebaseService
              .updateProject(project.id, { pouringTime: newPouringTime } as any)
              .catch((error) =>
                console.error(
                  "Error updating pouring time in Firebase:",
                  error,
            ...
            if (newPouringTime <= 0) {
              firebaseService
                ...
                .then(() =>
                  console.log(
                    `Project ${project.id} moved to Mixing2 in Firebase`,
                  ),
                )
                .catch((error) =>
                  console.error(
          ...
          if (
            ...
            if (newRemainingTime <= 0) {
              ...
              authService.getCurrentUserAsync().then((currentUser) => {
                if (currentUser) {
                  firebaseService
                    .updateCollaboratorBlocks(project.id, currentUser.uid, 1)
                    .catch((error) =>
                      console.error(
                        "Error updating collaborator blocks:",
                        error,
                      ),
                    );
                }
              ...
              if (completedBlocks >= project.blocks) {
                ...
                firebaseService
                  ...
                  .then(() => {
                    console.log(
                      `Project ${project.id} completed all ${project.blocks} blocks`,
                    );
                ...
                firebaseService
                  .updateProject(project.id, {
                    status: "Pouring",
                    pouringTime: 32, // 32 seconds for pouring phase (8s per material)
                    completedBlocks: completedBlocks,
                  ...
                  .then(() =>
                    console.log(
                      `Project ${project.id} starting block ${completedBlocks + 1} of ${project.blocks}`,
                    ),
                  )
                  .catch((error) =>
                    console.error(
        ...
        const hasActiveProject = updatedProjects.some(
          (p) =>
            p.status === "Pouring" ||
            p.status === "Mixing" ||
            p.status === "Pouring2" ||
            ...
            updatedProjects = updatedProjects.map((p) => {
              if (p.id === nextInQueue.id) {
                ...
                firebaseService
                  ...
                  .then(() =>
                    console.log(
                      `Project ${p.id} started Pouring block 1 of ${p.blocks} in Firebase`,
                    ),
                  )
                  .catch((error) =>
                    console.error(
      ...
      try {
        ...
        unsubscribe = onSnapshot(
          ...
          (snapshot) => {
            ...
            records.sort(
              (a, b) =>
                new Date(b.loadingDate).getTime() -
                new Date(a.loadingDate).getTime(),
            );
      ...
      try {
        ...
        unsubscribe = onSnapshot(
          ...
          (snapshot) => {
            ...
            records.sort(
              (a, b) =>
                new Date(b.backloadingDate).getTime() -
                new Date(a.backloadingDate).getTime(),
            );
  ...
  const loadProjects = async () => {
    try {
      ...
      if (!currentUser) {
        console.warn(
          "No authenticated user — cannot load projects from Firestore",
        );
        setProjects([]);
        return;
      }
      ...
      const updatedProjects: Project[] = projectsList.map((project) => {
        ...
        if (
          updatedProject.status === "Mixing" &&
          updatedProject.remainingTime === undefined
        ) {
          updatedProject = { ...updatedProject, remainingTime: 60 };
        }
        if (
          updatedProject.status === "Mixing2" &&
          updatedProject.remainingTime === undefined
        ) {
          updatedProject = { ...updatedProject, remainingTime: 26 };
        }
        if (
          updatedProject.status === "Pouring" &&
          updatedProject.pouringTime === undefined
        ) {
          updatedProject = {
            ...updatedProject,
            pouringTime: 66,
        ...
        if (
          updatedProject.status === "Pouring2" &&
          updatedProject.pouringTime === undefined
        ) {
  ...
  const addToInventory = async (
    ...
    try {
      ...
      console.log(
        `Adding to inventory: ${quantity} blocks from project "${projectName}"`,
      );
  ...
  const addLoadingRecord = async () => {
    try {
      if (
        ...
        Alert.alert(
          "Error",
          "Please fill in all required fields and select an inventory",
        );
        return;
      }
      ...
      const selectedInventory = inventory.find(
        (item) => item.id === selectedLoadingInventoryId,
      );
      ...
      if (blocksQty > selectedInventory.quantity) {
        Alert.alert(
          "Error",
          `Cannot load ${blocksQty} blocks. Only ${selectedInventory.quantity} blocks available in "${selectedInventory.projectName}".`,
        );
        return;
      }
      ...
      Alert.alert(
        "Success",
        `${blocksQty} blocks loaded out from "${selectedInventory.projectName}" successfully!`,
      );
    } catch (error: any) {
  ...
  const addBackloadingRecord = async () => {
    try {
      if (
        !backloadingContactName.trim() ||
        !backloadingAddress.trim() ||
        !backloadingBlocks.trim() ||
        !selectedBackloadingInventoryId
      ) {
        Alert.alert(
          "Error",
          "Please fill in all required fields and select a loading record",
        );
        return;
      }

      const blocksQty = parseInt(backloadingBlocks);
      const selectedLoadingRecord = loadingRecords.find(
        (record) => record.id === selectedBackloadingInventoryId,
      );
      ...
      if (
        !selectedLoadingRecord.inventoryId ||
        selectedLoadingRecord.inventoryId === ""
      ) {
        Alert.alert(
          "Error",
          "This loading record has no linked inventory. It may have been created before the inventory system was updated.",
        );
        return;
      }
      ...
      if (blocksQty > selectedLoadingRecord.blocks) {
        Alert.alert(
          "Error",
          `Cannot return ${blocksQty} blocks. Only ${selectedLoadingRecord.blocks} blocks in this loading record.`,
        );
        return;
      }
      ...
      const originalInventory = inventory.find(
        (item) => item.id === selectedLoadingRecord.inventoryId,
      );
      ...
      Alert.alert(
        "Success",
        `${blocksQty} blocks returned to "${originalInventory.projectName}" successfully!`,
      );
    } catch (error: any) {
  ...
  const deleteLoadingRecord = async (recordId: string) => {
    try {
      ...
      setLoadingRecords(
        loadingRecords.filter((record) => record.id !== recordId),
      );
  ...
  const deleteBackloadingRecord = async (recordId: string) => {
    try {
      ...
      setBackloadingRecords(
        backloadingRecords.filter((record) => record.id !== recordId),
      );
  ...
  const loadCompanyUsers = async () => {
    try {
      ...
      if (userProfile && (userProfile as any).company) {
        ...
        const users = await firebaseService.getCompanyUsersList(
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Success",
        editingProject
          ? "Tagumpay na nabago ang Proyekto!"
          : "Tagumpay na naidagdag ang Proyekto!",
      );
    } catch (error: any) {
        ...
        const updatedProjects: Project[] = projects.map((p) => {
          if (p.id === projectId) {
            return {
              ...
              pausedStatus: project.status as
                | "Pouring"
                | "Mixing"
                | "Pouring2"
                | "Molding"
  ...
  const handleManualUpDown = async (projectId: string) => {
    try {
      ...
      Alert.alert(
        "Success",
        `Tagumpay na nai-set ang pump sa ${newUp ? "UP" : "DOWN"}`,
      );
    } catch (error: any) {
  ...
  const toggleProjectStatus = async (projectId: string) => {
    try {
      ...
      if (newStatus === "Pouring") {
        ...
        if (hasActiveProject) {
          Alert.alert(
            "Hindi Makakapag-Start",
            "Mayroong ibang proyekto na kasalukuyang nasa proseso. Mangyaring maghintay hanggang matapos ito.",
          );
          return;
        }
      }
  ...
  const getStatusColor = (status: Project["status"]) => {
    switch (status) {
      case "Queue":
        return "#FF9900";
      case "Pouring":
        return "#9B59B6"; // Purple for pouring
      case "Mixing":
        return "#3498DB"; // Blue for mixing
      case "Pouring2":
        return "#9B59B6"; // Purple for second pouring
      case "Paused":
        return "#8E8E93"; // Gray for paused
      case "Completed":
        return "#00CC66";
      default:
        return "#FF9900";
  ...
  const checkMaterialsSufficient = (
    blocksToAdd: number,
  ): { sufficient: boolean; message: string; materials: any } => {
    ...
    const materials = {
      sand: {
        current: materialLevels.sand,
        needed: totalConsumption.sand,
        sufficient: materialLevels.sand >= totalConsumption.sand,
      },
      cement: {
        current: materialLevels.cement,
        needed: totalConsumption.cement,
        sufficient: materialLevels.cement >= totalConsumption.cement,
      },
      rha: {
        current: materialLevels.rha,
        needed: totalConsumption.rha,
        sufficient: materialLevels.rha >= totalConsumption.rha,
      },
      water: {
        current: materialLevels.water,
        needed: totalConsumption.water,
        sufficient: materialLevels.water >= totalConsumption.water,
      },
    };
  ...
  const handleAddBlocks = async () => {
    ...
    Alert.alert("Raw Materials Status", materialCheck.message, [
          ...
          if (materialCheck.sufficient) {
            try {
              ...
              const newBlocksCount =
                (selectedProject.blocks || 0) + blocksToAdd;
              ...
              Alert.alert(
                "Success",
                `Added ${blocksToAdd} blocks! Process started.`,
              );
            } catch (error: any) {
        ...
        [
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: () => {
              ...
              Alert.alert(
                "Success",
                "All materials have been restored to normal levels",
              );
            },
          },
        ],
        ...
        [
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: () => {
              ...
              Alert.alert(
                "Success",
                "All materials have been set to low levels",
              );
            },
          },
        ],
          ...
          <>
                ...
                <View style={styles.progressBarContainer}>
                  <View
                    style={[
                      styles.progressBar,
                      {
                        width: `${materialLevels.rha}%`,
                        backgroundColor: getMaterialColor(materialLevels.rha),
                      },
                    ]}
                  />
                </View>
                ...
                <View style={styles.progressBarContainer}>
                  <View
                    style={[
                      styles.progressBar,
                      {
                        width: `${materialLevels.sand}%`,
                        backgroundColor: getMaterialColor(materialLevels.sand),
                      },
                    ]}
                  />
                </View>
                ...
                <View style={styles.progressBarContainer}>
                  <View
                    style={[
                      styles.progressBar,
                      {
                        width: `${materialLevels.water}%`,
                        backgroundColor: getMaterialColor(materialLevels.water),
                      },
                    ]}
                  />
                </View>
            ...
            <View style={styles.projectsContainer}>
              <View style={styles.projectsHeader}>
                <ThemedText style={styles.projectsTitle}>
                  Nagawang Proyekto
                </ThemedText>
              ...
              {filteredProjects.length === 0 ? (
                <View style={styles.emptyContainer}>
                  ...
                  <ThemedText style={styles.emptyText}>
                    {searchQuery
                      ? "Walang nakitang proyekto na tumutugma sa iyong search."
                      : "Walang pang proyekto. Dagdagan mo ang iyong unang proyekto!"}
                  </ThemedText>
                </View>
              ) : (
                    ...
                    {project.status === "Mixing" &&
                      ...
                      project.remainingTime > 0 && (
                        <View style={styles.progressSection}>
                          ...
                          <ThemedText style={styles.progressText}>
                            Mixing block {(project.completedBlocks || 0) + 1}...{" "}
                            {Math.round(
                              ((60 - project.remainingTime) / 60) * 100,
                            )}
                            % complete
                          </ThemedText>
                        </View>
          ...
          <>
            ...
            <View style={styles.subTabContainer}>
              <Pressable
                ...
                style={[
                  styles.subTab,
                  inventorySubTab === "stock" && styles.activeSubTab,
                ]}
              >
                <ThemedText
                  style={[
                    styles.subTabText,
                    inventorySubTab === "stock" && styles.activeSubTabText,
                  ]}
                >
                  Stock
                </ThemedText>
              ...
              <Pressable
                ...
                style={[
                  styles.subTab,
                  inventorySubTab === "loading" && styles.activeSubTab,
                ]}
              >
                <ThemedText
                  style={[
                    styles.subTabText,
                    inventorySubTab === "loading" && styles.activeSubTabText,
                  ]}
                >
                  Loading
                </ThemedText>
              ...
              <Pressable
                ...
                style={[
                  styles.subTab,
                  inventorySubTab === "backloading" && styles.activeSubTab,
                ]}
              >
                <ThemedText
                  style={[
                    ...
                    <View style={styles.emptyContainer}>
                      ...
                      <ThemedText style={styles.emptyText}>
                        {inventorySearchQuery
                          ? "Walang katulad ang hinahanap mo sa inventory."
                          : "Wala pang Inventory, gumawa ng proyekto para makita ang inventory"}
                      </ThemedText>
                    </View>
                  ) : (
                    inventory
                        ...
                        <View key={item.id} style={styles.inventoryCard}>
                          <View style={styles.inventoryCardContent}>
                            <View style={styles.inventoryInfo}>
                              <ThemedText style={styles.inventoryProjectName}>
                                {item.projectName}
                              </ThemedText>
              ...
              <>
                ...
                <View style={styles.projectsContainer}>
                  <View style={styles.projectsHeader}>
                    ...
                    <ThemedText style={styles.projectsCount}>
                      {loadingRecords.length} record
                      {loadingRecords.length !== 1 ? "s" : ""}
                    </ThemedText>
                  </View>
                      ...
                      <View key={record.id} style={styles.auditCard}>
                        <View style={styles.auditCardContent}>
                          <View style={styles.auditInfo}>
                            ...
                            <ThemedText style={styles.auditAddress}>
                              {record.address}
                            </ThemedText>
                            ...
                            {record.notes && (
                              <ThemedText style={styles.auditNotes}>
                          ...
                          <View style={{ gap: 8 }}>
                            <Pressable
                              onPress={() =>
                                toggleLoadingStatus(record.id, record.status)
                              }
                              style={[
                                styles.deleteButton,
                                {
              ...
              <>
                ...
                <View style={styles.projectsContainer}>
                  <View style={styles.projectsHeader}>
                    ...
                    <ThemedText style={styles.projectsCount}>
                      {backloadingRecords.length} record
                      {backloadingRecords.length !== 1 ? "s" : ""}
                    </ThemedText>
                  </View>
                              ...
                              Alert.alert(
                                ...
                                [
                                  { text: "Isara", style: "cancel" },
                                  {
                                    ...
                                    style: "destructive",
                                    onPress: () =>
                                      deleteBackloadingRecord(record.id),
                                  },
                                ],
                              );
                            }}
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.modalBody}>
                ...
                <View style={styles.inputSection}>
                  <ThemedText style={styles.inputLabel}>
                    Ilan ang RHB na gusto mong gawin?
                  </ThemedText>
                  <TextInput
                ...
                <View style={styles.inputSection}>
                  <ThemedText style={styles.inputLabel}>
                    Estimated na oras para matapos (Auto-calculated):
                  </ThemedText>
                  <TextInput
                    ...
                    {showCollaboratorDropdown && (
                      <View style={styles.collaboratorDropdownMenu}>
                        {isLoadingUsers ? (
                          ...
                          <ThemedText style={styles.noCollaboratorsText}>
                            Walang ibang employee
                          </ThemedText>
                        ) : (
                          <ScrollView
                              ...
                              <Pressable
                                ...
                                style={[
                                  ...
                                  selectedCollaborators.includes(user.userId) &&
                                    styles.collaboratorItemSelected,
                                ]}
                                onPress={() => {
                      ...
                      <View style={styles.selectedCollaboratorsList}>
                        {selectedCollaborators.map((userId) => {
                          ...
                          return user ? (
                            <View
                              key={userId}
                              style={styles.selectedCollaboratorTag}
                            >
                              <ThemedText
                                style={styles.selectedCollaboratorText}
                              >
                                {user.fullName}
                              </ThemedText>
                              <Pressable
                  ...
                  <View style={styles.materialHeaderRow}>
                    <View style={styles.materialIconContainer}>
                      <Ionicons
                        name="leaf-outline"
                        size={24}
                        color={getMaterialColor(materialLevels.rha)}
                      />
                    </View>
                    <View style={styles.materialInfo}>
                      <ThemedText style={styles.materialLabelLarge}>
                        RHA
                      </ThemedText>
                      <ThemedText
                        style={[
                          styles.materialPercentage,
                          { color: getMaterialColor(materialLevels.rha) },
                        ]}
                      >
                        {materialLevels.rha}%
                  ...
                  <View style={styles.materialHeaderRow}>
                    <View style={styles.materialIconContainer}>
                      <Ionicons
                        ...
                        size={24}
                        color={getMaterialColor(materialLevels.sand)}
                      />
                    </View>
                    <View style={styles.materialInfo}>
                      <ThemedText style={styles.materialLabelLarge}>
                  ...
                  <View style={styles.materialHeaderRow}>
                    <View style={styles.materialIconContainer}>
                      <Ionicons
                        name="cube-outline"
                        size={24}
                        color={getMaterialColor(materialLevels.cement)}
                      />
                    </View>
                    <View style={styles.materialInfo}>
                      <ThemedText style={styles.materialLabelLarge}>
                  ...
                  <View style={styles.materialHeaderRow}>
                    <View style={styles.materialIconContainer}>
                      <Ionicons
                        name="water-outline"
                        size={24}
                        color={getMaterialColor(materialLevels.water)}
                      />
                    </View>
                    <View style={styles.materialInfo}>
                      <ThemedText style={styles.materialLabelLarge}>
          ...
          >
            <View style={styles.actionSheetContent}>
              ...
              <View style={styles.actionSheetHeader}>
                <ThemedText style={styles.actionSheetProjectName}>
                ...
                <View style={styles.actionSheetInfoGrid}>
                  <View style={styles.actionSheetInfoItem}>
                    ...
                    <ThemedText style={styles.actionSheetInfoLabel}>
                      Blocks
                    </ThemedText>
                    <ThemedText style={styles.actionSheetInfoValue}>
              ...
              <View style={styles.actionSheetButtons}>
                ...
                >
                  ...
                  />
                  <ThemedText
                    style={[styles.actionSheetButtonText, { color: "#34C759" }]}
                  >
                    Magdagdag ng Blocks
                  </ThemedText>
                </Pressable>
        ...
        >
          ...
          >
            <View style={styles.addBlocksModalContent}>
              <ThemedText style={styles.addBlocksTitle}>
                Magdagdag ng Blocks
              </ThemedText>

              <ThemedText style={styles.addBlocksLabel}>
                Ilang blocks ang gusto mong idagdag?
              </ThemedText>
              ...
              <View style={styles.addBlocksButtonsContainer}>
                ...
                >
                  <ThemedText style={styles.addBlocksCancelButtonText}>
                    Bumalik
                  </ThemedText>
                </Pressable>
                ...
                <Pressable
                  style={[
                    styles.addBlocksButton,
                    styles.addBlocksConfirmButton,
                  ]}
                  onPress={handleAddBlocks}
                >
                  <ThemedText style={styles.addBlocksConfirmButtonText}>
                    Magpatuloy
                  </ThemedText>
                </Pressable>
              </View>
              ...
              <View style={styles.modalBody}>
                <View style={styles.inputSection}>
                  ...
                  <Pressable
                    style={styles.collaboratorDropdown}
                    onPress={() =>
                      setShowLoadingInventoryDropdown(
                        ...
                        {inventory.length === 0 ? (
                          ...
                          inventory.map((item) => (
                            <Pressable
                              ...
                              style={[
                                styles.collaboratorItem,
                                selectedLoadingInventoryId === item.id &&
                                  styles.collaboratorItemSelected,
                              ]}
                              onPress={() => {
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.modalBody}>
                <View style={styles.inputSection}>
                  <ThemedText style={styles.inputLabel}>
                    Mag-select Loading Record (CLOSED):
                  </ThemedText>
                  <Pressable
                    style={styles.collaboratorDropdown}
                    onPress={() =>
                  ...
                  >
                    <ThemedText style={styles.collaboratorDropdownText}>
                      {selectedBackloadingInventoryId
                        ? (() => {
                            const record = loadingRecords.find(
                              (r) => r.id === selectedBackloadingInventoryId,
                            );
                            return record
                              ? `${record.contactName} (${record.blocks} blocks)`
                              : "Mag-select loading record";
                          })()
                  ...
                  {showBackloadingInventoryDropdown && (
                    <View style={styles.collaboratorDropdownMenu}>
                      <ScrollView style={styles.collaboratorsList}>
                        {loadingRecords.filter((r) => r.status === "CLOSED")
                          .length === 0 ? (
                          <ThemedText style={styles.noCollaboratorsText}>
                            No closed loading records available
                          </ThemedText>
                        ) : (
                          loadingRecords
                            ...
                            .map((record) => (
                              ...
                              >
                                ...
                                <View style={styles.collaboratorInfo}>
                                  ...
                                  <ThemedText style={styles.collaboratorEmail}>
                                    {record.blocks} RHB - {record.loadingDate}
                                  </ThemedText>
                                </View>
                              </Pressable>
                            ))
            ...
            <View style={styles.floatingHeader}>
              <View style={styles.floatingTitleSection}>
                <Ionicons
                  ...
                  style={{ marginRight: 8 }}
                />
                <ThemedText style={styles.floatingTitle}>
                ...
                style={[
                  styles.floatingStatusBadge,
                  {
                    backgroundColor: getStatusColor(
                      getActiveProject()?.status || "Queue",
                    ),
                  },
                ]}
              >
                <ThemedText style={styles.floatingStatusText}>
                  {getActiveProject()?.status}
                </ThemedText>
              </View>
            </View>
            ...
            <View style={styles.floatingDetails}>
              {getActiveProject()?.status === "Pouring" && (
                <View style={styles.floatingDetailRow}>
                  <Ionicons
                    ...
                    style={{ marginRight: 8 }}
                  />
                  <ThemedText style={styles.floatingDetailLabel}>
                    Pagbuhos:{" "}
                  </ThemedText>
                  <ThemedText style={styles.floatingDetailValue}>
                    {getMaterialDisplayName(
                      getCurrentMaterial(getActiveProject() || ({} as Project)),
                    )}
                  </ThemedText>
                  <ThemedText style={styles.floatingDetailTime}>
                    {" "}
                    ({getActiveProject()?.pouringTime}s)
                  </ThemedText>
                </View>
              )}
              ...
              {getActiveProject()?.status === "Mixing" && (
                <View style={styles.floatingDetailRow}>
                  ...
                  <ThemedText style={styles.floatingDetailTime}>
                    {" "}
                    ({getActiveProject()?.remainingTime}s)
                  </ThemedText>
                </View>
              )}
              ...
              <View style={styles.floatingDetailRow}>
                ...
                <ThemedText style={styles.floatingDetailValue}>
                  {(getActiveProject()?.completedBlocks || 0) + 1}/
                  {getActiveProject()?.blocks}
                </ThemedText>
              </View>
            </View>
          </View>
